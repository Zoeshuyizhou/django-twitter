...
class RedisHelper:
    ...
    def _load_objects_to_cache(cls, key, objects):
        ...
        # *serialized_list 解压缩这个list
        # conn.rpush 支持 conn.rpush(key,1,2,3,...)  把1，2，3，。。一个个value加进去的意思
        if serialized_list:
    ...
    def load_objects(cls, key, queryset):
        ...
        # cache hit，如果在 cache 里存在, 则直接拿出来，然后返回
        if conn.exists(key):
            # lrange 从0取到-1 =》取出全部的数据(list of string)
